...
`/axr-check` is scheduled for **Phase 2** of the rq-axr plugin. Once the eight dimension checkers land, this command will accept a single dimension id (for example `docs_context`, `tests_ci`, `safety_rails`) and re-run only that checker, updating the corresponding slice of `.axr/latest.json` without re-scoring the rest of the repo.
