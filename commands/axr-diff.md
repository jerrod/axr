...
`/axr-diff` is scheduled for **Phase 4** of the axr plugin. Once `.axr/history/` archival lands, this command will compare two runs (by sha, timestamp, or file path) and surface which dimensions moved, which criteria flipped, and which blockers were resolved or introduced.
