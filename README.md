...
Claude Code plugin (`rq-axr`) that scores a repository's **Agent eXecution Readiness** against a defined rubric.

Distributed alongside ARQU's `rq` plugin. Produces a human-readable report and a machine-readable JSON artifact per run, stored in `.axr/`.
