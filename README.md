...
Point Codex at the repo-root `codex/` directory — it is a self-contained Codex marketplace with `.agents/plugins/marketplace.json` and `plugins/<name>/` entries generated by `bin/build-codex`. Do NOT point Codex at `plugins/` (that layout is Claude-native and won't parse as a Codex marketplace).
