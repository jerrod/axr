...
for af in "${agent_files[@]}"; do
    ...
    while [ "$i" -lt "$crit_count" ]; do
        ...
        # Validate evidence is a JSON array of strings, max 20 elements, each ≤500 chars.
        ...
        ev_nonstring="$(jq '[.evidence[] | select(type != "string")] | length' <<<"$crit_json")"
        [ "$ev_nonstring" -eq 0 ] || die "agent criterion $crit_id evidence contains non-string elements in $af"
