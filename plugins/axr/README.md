...
Grades a repository against a 100-point rubric across 12 dimensions:

| Dimension | Weight | Question |
|---|---|---|
| Tests & CI Signal | 14 | Can the agent trust feedback when it changes code? |
| Docs & Agent Context | 14 | Does the repo explain itself to a cold-start agent? |
| Change Surface Clarity | 10 | Can the agent find the right place to change and understand the blast radius? |
| Safety Rails | 10 | What stops the agent when it is wrong? |
| Legibility | 8 | Can an agent read and navigate the codebase without human explanation? |
| Patterns | 8 | Does the codebase use one way to do each thing? |
| Style & Validation | 8 | Is code quality enforced mechanically before review? |
| Structure & Modularity | 6 | Can the agent change one thing without breaking ten? |
| Tooling & Reproducibility | 6 | Are local feedback loops fast, reproducible, unambiguous? |
| Supply Chain | 6 | Are dependencies managed, audited, and current? |
| Execution Visibility | 5 | Can the agent see what it did and what happened afterward? |
| Workflow Realism | 5 | Can the agent safely rehearse real work? |
...
**Rubric v4.0.** All 12 dimensions have deterministic checkers for mechanical criteria; 5 judgment subagents score the 17 judgment criteria. See `docs/plugin-brief.md` for the authoritative spec.
