...
_sdlc_git_root=$(git rev-parse --show-toplevel 2>/dev/null || echo "$PWD")
if [ -z "${SDLC_CONFIG_FILE:-}" ]; then
  SDLC_CONFIG_FILE=""
  if [ -f "$_sdlc_git_root/sdlc.config.json" ]; then
    SDLC_CONFIG_FILE="$_sdlc_git_root/sdlc.config.json"
  fi
fi
...
print(f'_SDLC_EXT_CONFIG={shlex.quote(json.dumps(merged_ext))}')
print(f'_SDLC_GLOBAL_CONFIG={shlex.quote(json.dumps(vals))}')
...
print(f'_SDLC_PATH_CONFIG={shlex.quote(json.dumps(merged_paths))}')

allow = config.get('allow', {})
print(f'_SDLC_ALLOW_CONFIG={shlex.quote(json.dumps(allow))}')

# QA config
qa_config = config.get('qa', {})
qa_defaults = {'enabled': True, 'timeout_seconds': 30, 'max_flows': 20}
qa_merged = {**qa_defaults, **qa_config}
print(f'_SDLC_QA_CONFIG={shlex.quote(json.dumps(qa_merged))}')
...
print(f'_SDLC_DESIGN_AUDIT_CONFIG={shlex.quote(json.dumps(da_merged))}')

# Commands config
commands = config.get('commands', {})
print(f'_SDLC_COMMANDS_CONFIG={shlex.quote(json.dumps(commands))}')
" 2>/dev/null) || {
  # If python3 fails, use env var fallbacks
  _sdlc_config_vars=""
}

if [ -n "$_sdlc_config_vars" ]; then
  # Strip carriage returns (CRLF platforms) and blank trailing lines before check.
  _sdlc_config_vars=$(echo "$_sdlc_config_vars" | tr -d '\r' | grep -v '^$' || true)
  ...
  _SDLC_PATH_CONFIG='{}'
  _SDLC_ALLOW_CONFIG='{}'
  read -r _SDLC_QA_CONFIG <<<'{"enabled":true,"timeout_seconds":30,"max_flows":20}'
  read -r _SDLC_DESIGN_AUDIT_CONFIG <<<'{"enabled":true,"min_grade":"C","skip_categories":[],"wcag_level":"AA"}'
  _SDLC_COMMANDS_CONFIG='{}'
fi
...
# threshold-helpers.sh consumers import it via sys.path insertion using $_SDLC_SCRIPT_DIR.
...
export _SDLC_ALLOW_CONFIG
export _SDLC_QA_CONFIG
export _SDLC_DESIGN_AUDIT_CONFIG
export _SDLC_COMMANDS_CONFIG
...
get_qa_config() {
  local key="$1"
  echo "$_SDLC_QA_CONFIG" | python3 -c "import sys,json; print(json.loads(sys.stdin.read()).get(sys.argv[1], ''))" "$key"
}

get_design_audit_config() {
  local key="$1"
  echo "$_SDLC_DESIGN_AUDIT_CONFIG" | python3 -c "import sys,json; print(json.loads(sys.stdin.read()).get(sys.argv[1], ''))" "$key"
}
