...
> This is the authoritative specification for the `rq-axr` plugin. The plan in `.quality/plans/` references this document for rubric content and scoring anchors.

## Context

Building `rq-axr`, a Claude Code plugin that scores a repository's **Agent eXecution Readiness (AXR)** against a defined rubric. Distributed org-wide at ARQU alongside the existing `rq` plugin via managed settings. v1 of a larger initiative — a GitHub App will follow later, so scoring logic should be extractable.
      ...
      "criteria": [
        {
          ...
          "notes": "Flaky tests in tests/integration/test_atlas_sync.py occasionally add 2–3min",
          "reviewer": "agent"
        }
      ]
    }
...
ARQU repos are primarily Kotlin and Python (FastAPI) with Vue frontends — prioritize those. Fall back to language-agnostic checks on unsupported stacks and note the limitation.
...
4. Run against one ARQU repo, inspect output, tune.
...
- Phase 5: Calibration pilot on two ARQU repos
- Phase 6: Org distribution via managed settings

## Success criteria for v1

1. `/axr` produces a complete report on a top ARQU repo in <20 min of tool-use time.
